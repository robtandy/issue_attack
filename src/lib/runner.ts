...
import { checkCommand, violationMessage, PolicyResult } from "./policy.js";
...
function isSupervisorComment(body: string | undefined | null): boolean {
  const b = String(body ?? "");
...
interface IssueRecord {
  ...
  url: string;
  body?: string;
  comments?: Array<{ id?: number; createdAt?: string; author?: { login: string }; body?: string }>;
...
export class AgentRunner {
  private root: string;
  private repo: string;
  private repoInfo: { nameWithOwner: string; defaultBranch: string };
  private config: Config;
  private issue: number;
  private mode: "run" | "resume" | "fresh";
  private model: string | null;
  private onLine?: (msg: string) => void;
  private streamText: boolean;

  private startedAt: number;
  private outcome: Outcome | null;
  private violations: number;
  private stopRequested: boolean;
  private softSent: boolean;
  private statusCommentId: number | null;
  private lastAction: string | null;
  private seenCommentIds: Set<string>;
  private ownCommentIds: Set<number>;
  private commentPollAt: number;
  private lastAgentActivityAt: number;
  private lastPersistedActivity: number;
  private state: stateMod.State;
  private entry: stateMod.StateEntry | null;
  ...
  private finalStats?: SessionStats | null;
  ...
  private updateEntry(patch: Partial<stateMod.StateEntry>): stateMod.StateEntry {
    this.state = stateMod.loadState(this.root);
  ...
  async run(): Promise<Outcome> {
    try {
      ...
      if (this.issueRecord.state !== "OPEN")
        throw new SkipIssue(`issue is ${this.issueRecord.state}`);
      if (await gh.isPullRequest(this.root, this.repo, this.issue)) {
        throw new SkipIssue(
          `#${this.issue} is a pull request, not an issue (they share a numbering space)`
        );
      }
      ...
      if (error.code === "SKIP") {
        this.outcome = { kind: "skipped", reason: error.message };
      } else {
        this.outcome = { kind: "error", reason: (error as Error).message };
        this.line(`${c.red("error:")} ${(error as Error).message}`);
      }
    }
  ...
  private async releaseClaim({ removeLabels = [] } = {}): Promise<void> {
  ...
  private async startClient(): Promise<void> {
    ...
    const sessionId =
      this.mode === "fresh" || !this.entry?.sessionId
        ...
        : (this.entry.sessionId as string);
    ...
    this.client = new PiClient({
      ...
      onRecord: (rec: PiRecord) => this.handleRecord(rec),
  ...
  private async sendTask(): Promise<void> {
    ...
    if (this.mode === "resume" && this.entry) {
      ...
      const newComments = (this.issueRecord!.comments ?? []).filter(
        (cm) => (cm.createdAt ?? "") > since
      );
      ...
      promptText = resumePrompt({
        ...
        blockedNote,
        newComments: newComments as any,
        base: this.base!,
        branch: this.branch!,
  ...
  private handleRecord(rec: PiRecord): void {
    try {
    ...
    if (rec.type === "message_end" && (rec as any).message?.role === "assistant") {
      this.lastAgentActivityAt = Date.now();
      return;
    }
  ...
  private handleViolation(v: PolicyResult): void {
    this.violations++;
    this.line(`${c.red(`policy violation (${v.rule}):`)} ${v.reason}`);
    if (this.violations === 1) {
      this.client!.message(violationMessage(v)).catch((e) =>
  ...
  private async monitorLoop(): Promise<Outcome> {
    const tickMs = 2_000;
    ...
    while (true) {
      ...
      if (this.client!.exitInfo) {
        return {
          kind: "failed",
          reason: `pi process exited (code=${this.client!.exitInfo.code})`,
          lastText: null,
        };
      }
      ...
      const costOver =
        ...
        usage.cost != null &&
        usage.cost >= this.config.costBudgetUsd;
      const costSoft =
        this.config.costBudgetUsd &&
        usage.cost != null &&
      ...
      const tokenOver =
        ...
        usage.tokens != null &&
        usage.tokens >= this.config.maxTokens;
      ...
      if (
        (this.config.commentSteerSeconds ?? 0) > 0 &&
      ...
      if (this.client!.settledCount > handledSettled) {
        ...
        if (!st || (!st.isStreaming && ((st as any).pendingMessageCount ?? 0) === 0)) {
          ...
          if (
            ...
            this.line(
              c.yellow("no PR and no BLOCKED.md — auto-retry with a continue prompt")
            );
            this.attempt!++;
            this.updateEntry({ attempts: this.attempt });
            await this.client!.message(
              continuePrompt({ issue: this.issue, reason: result.reason! })
            );
            continue;
          }
  ...
  private async waitSettle(timeoutMs: number): Promise<void> {
    const target = this.client!.settledCount + 1;
    const deadline = Date.now() + timeoutMs;
    while (
      Date.now() < deadline &&
      this.client!.settledCount < target &&
      !this.client!.exitInfo
    ) {
      await sleep(500);
    }
  ...
  private async pollComments(): Promise<void> {
    ...
    for (const cm of rec.comments ?? []) {
      const key = (cm.id ?? `${cm.createdAt}|${cm.author?.login ?? ""}`) as any;
      if (this.seenCommentIds.has(key)) continue;
      this.seenCommentIds.add(key);
      if (this.ownCommentIds.has(key as any)) continue; // ours (heartbeat/outcome)
      if (isSupervisorComment(cm.body)) continue; // belt & suspenders
      if ((cm.createdAt ?? "") <= startedIso) continue; // predates this run
      fresh.push(cm);
    }
    ...
    const text = fresh
      .map(
        (cm) =>
          `[ISSUE COMMENT] ${cm.author?.login ?? "unknown"} wrote:\n${String(cm.body).slice(0, 2000)}`
      )
      .join("\n\n");
    this.line(
      `${c.magenta("issue comment")} from ${fresh.map((f) => f.author?.login).join(", ")} — steering agent`
    );
    await this.client.message(text).catch((e) => this.line(`steer failed: ${(e as Error).message}`));
  }

  private async classify(): Promise<Outcome> {
    ...
    return {
      kind: "failed",
      reason: "ended without opening a PR or writing BLOCKED.md",
      lastText,
    };
  }
  ...
  private async deleteStatusComment(): Promise<void> {
  ...
  private async postOutcome(kind: string): Promise<void> {
   ...
   */
  private async enforcePrConventions(): Promise<void> {
  ...
  private async finalize(): Promise<void> {
    ...
    if (kind === "succeeded") {
      ...
      await safe("label PR", async () => {
        const created = await gh
          .addPrLabel(this.root, this.repo, this.outcome!.prNumber!, this.config.prLabel)
          .catch(() => false);
        if (!created) {
          await gh.ensureLabel(
            this.root,
    ...
    this.updateEntry({
      ...
      conflicts: this.outcome?.conflicts ?? false,
      cost: this.finalStats?.cost ?? null,
      tokens: (this.finalStats?.tokens as any)?.total ?? null,
      sessionId: this.sessionId ?? (this.entry?.sessionId as string) ?? null,
      piPid: null,
...
export async function attackFleet({
  ...
  const active = new Map<number, { done: boolean; runner: AgentRunner }>();
  let stopping = false;
  let wake: (() => void) | null = null;

  const fleetLine = (n: number, msg: string) =>
    out(`[${c.bold(`#${n}`)}] ${msg}`);
  ...
  const launch = async (issueNumber: number) => {
  ...
  out(
    `attacking ${c.bold(repo)} — label: ${c.cyan(label)} max: ${max}${watch ? " (watch mode)" : ""}`
  );
