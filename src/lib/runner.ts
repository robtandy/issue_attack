...
import type { RepoInfo, IssueRecord, IssueComment, ListIssueRecord } from "./gh.js";
...
import type { PiRecord, PiModel } from "./pi-client.js";
import {
  workerContract,
  taskPrompt,
  resumePrompt,
...
export function resolveModelSpec(spec: string, models: PiModel[]): PiModel | null {
  const s = String(spec);
...
const modelLabel = (m: PiModel | null | undefined): string | null =>
  m?.provider ? `${m.provider}/${m.id}` : m?.id ?? null;

/** True for comments generated by the supervisor (heartbeats, outcomes). */
function isSupervisorComment(body: string | null | undefined): boolean {
  const b = String(body ?? "");
 ...
 */
export function unaddressedComments(
  comments: IssueComment[] | undefined,
  ackedIds: string[] | undefined
): IssueComment[] {
...
export class AgentRunner {
  ...
  stopRequested = false;
  softSent = false;
  statusCommentId: string | null = null;
  lastAction: string | null = null;
  seenCommentIds = new Set<string>(); // issue comments already handled (steer de-dup)
  ownCommentIds = new Set<string>(); // supervisor-generated comment ids (never steer)
  commentPollAt = 0;
  ...
  line(msg: string): void {
    (this.onLine ?? ((m: string) => console.log(m)))(msg);
  }

  /** Reload-merge-save: fleet runners must not clobber each other's entries. */
  updateEntry(patch: Partial<StateEntry>): StateEntry {
    this.state = stateMod.loadState(this.root);
  ...
  async run(): Promise<Outcome> {
    try {
      ...
      if (e.code === "SKIP") {
        this.outcome = { kind: "skipped", reason: e.message };
      } else {
        this.outcome = { kind: "error", reason: e.message };
        this.line(`${c.red("error:")} ${e.message}`);
      }
    }
    ...
    if (this.client && !this.client.exitInfo) {
      ...
      if (!this.outcome?.lastText) {
        this.outcome = {
          ...this.outcome,
          lastText: await this.client.lastAssistantText().catch(() => null),
        };
      }
    }
    await this.finalize();
  ...
  async claim(): Promise<void> {
    ...
    if (a2.some((a) => a !== me)) {
      await gh
        .editIssue(this.root, this.repo, this.issue, {
          issue: after,
          removeAssignee: me,
  ...
  async releaseClaim({ removeLabels = [] }: { removeLabels?: string[] } = {}): Promise<void> {
    ...
    try {
      ...
      this.line(`${c.yellow("warn:")} could not release claim: ${(err as Error).message}`);
    }
  }
  ...
  async applyModelSpec(spec: string): Promise<PiModel | null> {
    const { modelPart, level } = parseModelSpec(spec);
    let resolved: PiModel | null = null;
    try {
      ...
      this.line(`${c.yellow("warn:")} could not apply model "${modelPart}": ${(e as Error).message}`);
      return null;
    }
    if (level) {
      try {
        ...
        this.line(`${c.yellow("warn:")} ${resolved.id} rejected thinking level "${level}": ${(e as Error).message}`);
      }
    }
    return resolved;
  }
  ...
  async sendTask(): Promise<void> {
    ...
    let ackComments: IssueComment[] = []; // resume-mode comments handed to the agent below
    if (this.mode === "resume" && this.entry) {
      ...
      let newPrComments: IssueComment[] = [];
      if (this.entry.prNumber) {
        try {
          ...
          this.line(`${c.yellow("warn:")} could not fetch PR #${this.entry.prNumber} comments: ${(err as Error).message}`);
    ...
    for (const cm of ackComments) {
      try {
        ...
        this.line(`${c.dim("note:")} could not acknowledge comment: ${(e as Error).message}`);
      }
    }
  ...
  handleRecord(rec: PiRecord): void {
    try {
  ...
  handleViolation(v: PolicyViolation): void {
    this.violations++;
    this.line(`${c.red(`policy violation (${v.rule}):`)} ${v.reason}`);
    if (this.violations === 1) {
      this.client!.message(violationMessage(v)).catch((e) => this.line(`steer failed: ${(e as Error).message}`));
    } else {
  ...
  async monitorLoop(): Promise<Outcome> {
    const tickMs = 2_000;
    ...
    while (true) {
      ...
      for (const s of stateMod.takeSteer(this.root, this.issue)) {
        ...
        await this.client!.message(`[OPERATOR GUIDANCE] ${s.message}`).catch((e) =>
          this.line(`steer failed: ${(e as Error).message}`)
        );
      }
      ...
      const usage = {
        cost: (stats?.cost as number | undefined) ?? null,
        tokens: ((stats?.tokens as { total?: number } | undefined)?.total) ?? null,
      };
      ...
      if (this.config.heartbeatMinutes > 0 &&
        ...
        await this.updateStatusComment("running", usage).catch((e) =>
          this.line(`${c.yellow("warn:")} heartbeat comment failed: ${(e as Error).message}`)
        );
      }
      ...
      if ((this.config.commentSteerSeconds ?? 0) > 0 &&
        ...
        await this.pollComments().catch((e) =>
          this.line(`${c.yellow("warn:")} comment poll failed: ${(e as Error).message}`)
        );
      }
  ...
  async waitSettle(timeoutMs: number): Promise<void> {
  ...
  async pollComments(): Promise<void> {
    ...
    const fresh: Array<{ cm: IssueComment; key: string }> = [];
  ...
  async classify(): Promise<Outcome> {
  ...
  async updateStatusComment(
    stateLabel: string,
    usage: { cost: number | null; tokens: number | null },
    extra?: string
  ): Promise<void> {
  ...
  async deleteStatusComment(): Promise<void> {
  ...
  async postOutcome(kind: string): Promise<void> {
    const body = outcomeComment({
      ...
      attempt: this.attempt,
      cost: (this.finalStats?.cost as number | undefined) ?? null,
      elapsedMs: Date.now() - this.startedAt,
   ...
   */
  async enforcePrConventions(): Promise<void> {
    ...
    try {
      ...
      this.line(`${c.yellow("warn:")} PR conventions: ${(err as Error).message}`);
    }
  }
  ...
  async finalize(): Promise<void> {
    ...
    const safe = async (label: string, fn: () => Promise<void>): Promise<void> => {
      try {
        ...
        this.line(`${c.yellow("warn:")} ${label} failed: ${(err as Error).message}`);
      }
    };
    ...
    this.updateEntry({
      ...
      conflicts: this.outcome?.conflicts ?? false,
      cost: (this.finalStats?.cost as number | undefined) ?? null,
      tokens: ((this.finalStats?.tokens as { total?: number } | undefined)?.total) ?? null,
 ...
 */
export async function attackFleet({
  root,
  repoInfo,
  config,
  max,
  ...
  model,
  out,
}: FleetOptions): Promise<void> {
  const repo = repoInfo.nameWithOwner;
  const active = new Map<number, { done: boolean; runner: AgentRunner }>(); // issueNumber -> {done, runner}
  let stopping = false;
  let wake: (() => void) | null = null;

  const fleetLine = (n: number, msg: string): void => out(`[${c.bold(`#${n}`)}] ${msg}`);
  ...
  const nextClaimable = async (): Promise<ListIssueRecord | null> => {
    ...
    const issues = await gh.listIssues(root, repo, label, limit ?? 100).catch((e) => {
      out(c.yellow(`warn: issue list failed: ${(e as Error).message}`));
      return [];
    });
  ...
  const launch = async (issueNumber: number): Promise<void> => {
    ...
    runner.run().then((out2) => {
      ...
      fleetLine(issueNumber, c.red(`runner crashed: ${(err as Error).message}`));
      slot.done = true;
    });
  };
