...
/** Add an emoji reaction to a comment (e.g., "eyes" for 👀 or "eyes_look" for acknowledgment). */
export async function addReactionToComment(root, repo, commentId, emoji) {
  const { code, stderr } = await exec(
    "gh",
    ["api", `repos/${repo}/issues/comments/${commentId}/reactions`, "-X", "POST", "--input", "-"],
    { cwd: root, input: JSON.stringify({ content: emoji }) }
  );
  // 201 (created) or 200 (already existed) are both ok; 422 is also ok (duplicate/invalid).
  if (code !== 0 && code !== 201 && code !== 200 && code !== 422) {
    throw new Error(`gh add reaction: ${stderr.trim()}`);
  }
}
