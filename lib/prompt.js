...
export function resumePrompt({ issue, prevStatus, blockedNote, newComments, newPrComments, base, branch, attempt, prHasConflicts }) {
  ...
  const conflictNote = prHasConflicts
    ? `\n\n**Your open pull request has merge conflicts with the base branch \`${base}\`** — the\nbase branch moved after the PR opened, and resolving those conflicts is your first job\nthis run:\n1. \`git fetch origin\`\n2. \`git merge origin/${base}\` (merge only — never rebase, never force-push)\n3. Resolve the conflicts, preserving both your change and the base changes; re-run the tests\n4. \`git push origin ${branch}\` (normal push — the PR updates itself)\n\nIf a conflict genuinely cannot be resolved without maintainer input, write BLOCKED.md\ndescribing it exactly and stop.`
    : "";
...
`
  ...
  const baseDriftSection = prHasConflicts
    ? "" // the conflict callout above is the definite version — no second, conditional one
...
${newBlock}${prCommentSection}${baseDriftSection}
