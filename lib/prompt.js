...
export function resumePrompt({ issue, prevStatus, blockedNote, newComments, base, branch, attempt, prHasConflicts }) {
...
export function prFooter({ issue, attempt }) {
  return `---
🤖 This PR was opened by an [issue_attack](${TOOL_URL}) agent autonomously working issue #${issue}${attempt ? ` (attempt ${attempt})` : ""}. Review with the same care you would any contribution — verify the changes yourself.`;
}
...
export function outcomeComment({ outcome, issue, prUrl, blockedBody, lastText, attempt, cost, elapsedMs, logPath, worktree, retrying, conflicts }) {
  ...
  if (logPath) parts.push("", `<sub>run log: ${logPath}</sub>`, `<sub>via [issue_attack](${TOOL_URL}) — autonomous agents for GitHub issues</sub>`);
