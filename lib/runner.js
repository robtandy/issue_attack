...
export class AgentRunner {
    ...
    if (this.mode === "resume" && this.entry) {
      ...
      if (this.entry.prNumber) {
        try {
          ...
          // Acknowledge each new PR comment with an emoji reaction.
          for (const cm of newPrComments) {
            await gh.addReactionToComment(this.root, this.repo, cm.id, "eyes").catch((e) =>
              this.line(`${c.dim("note:")} could not acknowledge PR comment: ${e.message}`)
            );
          }
        } catch (err) {
      ...
      // Acknowledge each new issue comment with an emoji reaction.
      for (const cm of newComments) {
        await gh.addReactionToComment(this.root, this.repo, cm.id, "eyes").catch((e) =>
          this.line(`${c.dim("note:")} could not acknowledge issue comment: ${e.message}`)
        );
      }
   ...
   * 
   * Acknowledge each processed comment with an emoji reaction.
   */
  async pollComments() {
    const rec = await gh.viewIssue(this.root, this.repo, this.issue);
    ...
    for (const cm of rec.comments ?? []) {
      ...
      if (this.ownCommentIds.has(key)) continue; // ours (heartbeat/outcome)
    ...
    const text = fresh
      ...
      .join("\n\n");
    this.line(`${c.magenta("issue comment")} from ${fresh.map((f) => f.author?.login).join(", ")} — steering agent`);
    await this.client.message(text).catch((e) => this.line(`steer failed: ${e.message}`));
    // Acknowledge each processed comment with an emoji reaction.
