...
async function doPublish(root, repoInfo, config, force) {
  ...
  // Acquire cross-process lock to coordinate status page publishes.
  let releaseLock = null;
  try {
    releaseLock = stateMod.acquireLock(root, "status-publish", 30_000);
  } catch (err) {
    // Lock acquisition timeout: skip this publish; another process has it.
