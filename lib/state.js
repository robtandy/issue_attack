...
/**
 * Acquire a lock for a named resource (blocking). Used for coordinating
 * cross-process operations like status page publishing. Returns a release
 * function that must be called when done.
 */
export function acquireLock(root, name, timeoutMs = 30_000) {
  const lockFile = join(dirs(root).base, `.lock-${name}`);
  const deadline = Date.now() + timeoutMs;
  
  while (Date.now() < deadline) {
    // Check if lock file exists; if not, try to create it atomically
    if (!existsSync(lockFile)) {
      try {
        const tmp = lockFile + "." + process.pid + "." + Math.random().toString(36);
        ...
        // Rename failed: another process beat us to it
      }
    }
    
    // Lock is held by another process; poll briefly and retry
    const remaining = deadline - Date.now();
    if (remaining <= 0) break;
    // Busy-wait in small increments
    const pollMs = Math.min(10, remaining);
    const start = Date.now();
    while (Date.now() - start < pollMs) {}
  }
  
  throw new Error(`failed to acquire lock '${name}' after ${timeoutMs}ms`);
}
